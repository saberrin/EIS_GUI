...
class HeatMap3DWidget:
    ...
    def render_and_save(self, save_path, render_width=5120, render_height=2880, high_quality=True):
        """
        ...
        Args:
            ...
            high_quality (bool): Whether to use anti-aliasing for improved clarity.
        """
        ...
        # Enable anti-aliasing for high-quality rendering
        if high_quality:
            plotter.enable_anti_aliasing("ssaa")  # Super-Sampling Anti-Aliasing (SSAA)

        # Save the image at the specified resolution
    ...
    def create_responsive_label(self, save_path, layout, crop_values=None):
        ...
        # Render and save the heatmap with high resolution
        render_width, render_height = 5120, 2880  # 5K resolution
        self.render_and_save(save_path, render_width, render_height)

        # Load the rendered image using PIL
        image = Image.open(save_path)

        # Apply cropping based on the provided crop values
        left = crop_values.get("left", 0)
        right = crop_values.get("right", 0)
        top = crop_values.get("top", 0)
        bottom = crop_values.get("bottom", 0)
        ...
        # Save the cropped image for use in QLabel
        cropped_path = save_path.replace(".png", "_cropped.png")
        cropped_image.save(cropped_path)

        # Load the cropped image into QLabel
        label = QLabel()
        pixmap = QPixmap(cropped_path)

        # Resize the pixmap to fit within the layout while keeping aspect ratio
